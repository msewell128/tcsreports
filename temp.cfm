			...
			<!--- // Children of Epic // --->
					...
					<cfif aChildren.recordcount>
						...
						<cfscript>
							...
							if(aChildren.updated GT maxUpdatedDate)
								maxUpdatedDate = aChildren.updated;
						</cfscript>
						...
						<cfif bChildren.recordcount>
							<cfloop query="bChildren">
								<cfscript>
									...
									if(bChildren.updated GT maxUpdatedDate)
										maxUpdatedDate = aChildren.updated;
				...
				<cfset timeRemaining = totOEst-totTimeSpent>
				<cfsavecontent variable="eSummary">
				<thead>
					<tr>
						<td scope="col" style="font-weight: 900;">#currentrow#</td>
						<td scope="col" style="font-weight: 900;">#EpicName# [<a href='###replaceNoCase(EpicName," ","_","all")#'>Detail</a>] [<a href="http://issues.tcs/browse/DEV-#issueNum#" target="_blank">DEV-#issueNum#</a>]
						<br>Hours :: #val(is_Null(totOEst)/60/60)# | #val(is_Null(timeRemaining)/60/60)# | #val(is_Null(totTimeSpents)/60/60)# | #val(is_Null(totHrsRemaining)/60/60)# |
						<br>Days :: #val(is_Null(totOEst)/60/60/8)# | #val(is_Null(timeRemaining)/60/60/8)# | #val(is_Null(totTimeSpents)/60/60/8)# | #val(is_Null(totHrsRemaining)/60/60/8)# | </td>
<!--- Estimated --->
						<cfif totHrsRemaining GT timeRemaining>
							<td>#numberFormat(val(is_Null(totOEst+(totHrsRemaining-timeRemaining))/60/60/8),"999.99")#a</td>
						<cfelseif totHrsRemaining LT (totOEst-totTimeSpent)>
							<td>#numberFormat(val(is_Null(totOEst-totHrsRemaining)/60/60/8),"999.99")#b</td>
						<cfelse>
							<td>#numberFormat(val(is_Null(totOEst)/60/60/8),"999.99")#c</td>
						</cfif>
					<!--- 	<td>#val(is_Null(totOEst)/60/60/8/5)#</td> --->
						<td><!--- #dateFormat(maxUpdatedDate,"MM/DD/YY")# ---></td>
<!--- Remaining --->
						<td>#numberFormat(val(is_Null(totHrsRemaining)/60/60/8),"999.99")#</td>
						<!--- <td>#val(is_Null(totTimeSpents)/60/60)#</td> --->
						<td>Date Here?</td>
<!--- Completed	--->
						<td>#numberFormat(val(is_Null(totTimeSpent)/60/60/8),"999.99")#</td>
			...
			<table class="tsc_table_s3" summary="Summary" style="width:100%;">
				<thead>
					<tr>
						...
						<th scope="col" colspan="2">Estimated</th>
						<th scope="col" colspan="2">Remaining</th>
